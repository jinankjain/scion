...
func (c *Certificate) Sign() error {
	sign, err := crypto.Sign(c.Bytes(), ApnaMSConfig.Privkey, crypto.Ed25519)
	if err != nil {
		return err
	}
	c.Signature = sign
...
func (c *Certificate) Verify() error {
	return crypto.Verify(c.Bytes(), c.Signature, ApnaMSConfig.Pubkey, crypto.Ed25519)
}
...
func NewCertificateFromRawBytes(raw common.RawBytes) (*Certificate, error) {
	...
	cert := &Certificate{
		...
		ExpTime:   raw[ExpTimeOffset:(ExpTimeOffset + apna.TimeLen)],
